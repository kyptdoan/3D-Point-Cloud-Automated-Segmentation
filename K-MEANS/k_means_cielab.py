...
def load_as_o3d_cloud(path):
    ...
    spatial_coordinate = data[:,0:3]
    rgb = data[:,3:6] / 255.0
    # Convert RGB to CIELAB
    cielab = rgb2lab(rgb)
    # Concatenate spatial and cielab colour attributes
    cielab_data = np.hstack((spatial_coordinate, cielab))
    pc = o3d.geometry.PointCloud()
    pc.points = o3d.utility.Vector3dVector(cielab_data[:,0:3])
    pc.colors = o3d.utility.Vector3dVector(cielab_data[:,3:6])
    colors = np.asarray(pc.colors)
    return data, spatial_coordinate, cielab_data, pc, np.float32(colors)
...
def visualise_segmented_pc(data ,segmented_labels, pc, k):
...
if __name__ == '__main__':
    ...
    annotated_data, spatial_coordinate, cielab_data, pc, colors = load_as_o3d_cloud(data_dir + file_name)
    ...
    if segmentation_type == '1': 
        ...
        visualise_segmented_pc(annotated_data, segmented_labels, pc, 9)
        print(segmented_labels)
        #save_data_as_xyz(cielab_data, segmented_labels, "C:\\Users\\LENOVO\\Desktop\\Project\\Strawberry-Project-\\K-MEANS\\CIELAB_iou\\A2_20220608_lab_kmeans_semantic.xyz")
        ...
        visualise_segmented_pc(annotated_data, segmented_labels, pc, k)
        print(k)
        print(segmented_labels)
        # save_data_as_xyz(cielab_data, segmented_labels, "C:\\Users\\LENOVO\\Desktop\\Project\\Strawberry-Project-\\K-MEANS\\CIELAB_iou\\A2_20220608_lab_kmeans_instance.xyz")
